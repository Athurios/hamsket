	...
	,data: [
		{
			...
			,type: 'messaging'
			,js_unread: `let checkUnread=()=>{const elements=document.querySelectorAll("#pane-side .P6z4j, .unread");let count=0;for(let i of elements)0===i.querySelectorAll('#pane-side *[data-icon="muted"]').length&&count++;hamsket.updateBadge(count)};setInterval(checkUnread,1e3);let unregister_queue=[];navigator.serviceWorker.getRegistrations().then(registrations=>{for(const registration of registrations)unregister_queue.push(registration.unregister());return unregister_queue}).then(queue=>{}).catch(err=>{});`
		},
		{
			 id: 'slack'
			,logo: 'slack.png'
